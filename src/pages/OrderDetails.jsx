    ...
    const loadOrder = async () => {
        ...
        try {
            ...
            if (found) {
                ...
                const currentTotals = found.totals || { ht: (found.articles || []).reduce((sum, a) => sum + (a.total || 0), 0), tva: 0, ttc: found.amount || 0 };
                if (!found.totals && currentTotals.ht > 0) {
                    currentTotals.tva = Math.round(currentTotals.ht * 0.19 * 100) / 100;
                    currentTotals.ttc = Math.round((currentTotals.ht + currentTotals.tva) * 100) / 100;
                }
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <input
                                                ...
                                                onChange={e => {
                                                    ...
                                                    const ttc = Math.round((parseFloat(tempHt) + tva) * 100) / 100;
                                                    setTempTva(tva);
                                                    setTempAmount(ttc.toString());
                                                }}
                                                className="w-full p-3 bg-white border-2 border-emerald-100 rounded-xl font-black text-slate-900 outline-none focus:border-emerald-500 shadow-sm"
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex gap-8">
                                        <div>
                                            ...
                                            <span className="text-xl font-black text-slate-900">{formatAmount(order.totals?.ht || (order.articles || []).reduce((sum, a) => sum + (a.total || 0), 0))}</span>
                                        </div>
                                        <div>
                                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 block">TVA</span>
                                            <span className="text-xl font-black text-slate-500">{formatAmount(order.totals?.tva || 0)}</span>
                                        </div>
                                        <div>
                                    ...
                                    {canEditAmount() && (
                                        <button onClick={() => {
                                            const ht = order.totals?.ht || (order.articles || []).reduce((sum, a) => sum + (a.total || 0), 0) || 0;
                                            const tva = order.totals?.tva || 0;
                                            setTempHt(ht);
                                            setTempTva(tva);
